...
Start the host daemon. This creates `~/.config/fcp/auth-token` if it does not already exist.

```bash
./build/fcp-$(go env GOOS)-$(go env GOARCH) ensure
```

Build or copy a Linux `fcp` binary for the container, make the host auth token available there, then start the container daemon:
...
export FCP_AUTH_TOKEN='<contents of ~/.config/fcp/auth-token from the host>'
...
| `--auth-token` / `--auth-token-file` | auto | Shared token used to register with the host daemon. |
...
| `FCP_AUTH_TOKEN` | empty | host, container, selected CLI commands | 64-character hex shared secret. |
...
2. `--auth-token-file`
3. `FCP_AUTH_TOKEN`
4. `FCP_AUTH_TOKEN_FILE`
5. `/run/secrets/fcp-auth-token`
6. `~/.config/fcp/auth-token` (auto-generated by `fcp ensure` or `fcp host-daemon`)

The token is required for container registration and daemon shutdown. Do not expose the control (`19285`) or data (`19286`) ports to untrusted networks.
